...
console.log(
  chalk.red(
    "Welcome Wanderer!! I am a wizard who will accompany you from now on."
  )
);
...
if (player.confirm) {
  ...
  status();
  console.log(
    `Your journey from your tavern, back to your village starts at dawn the next day.`
  );
  let player1 = await inquirer.prompt({
    name: "choice1",
    type: "list",
    message:
      "As you move through the forest you encounter a pack of wolves\nYou wish to: ",
    choices: ["Flee", "Fight"],
  });
  console.log("\n");
  ...
  if (player1.choice1 === "Fight") {
    for (
      wolves && player_status.health;
      wolves >= 0 && player_status.health >= 0;
      wolves && player_status.health
    ) {
    ...
    if (num <= 3) {
      ...
      console.log(
        `You try to sneak away quitely, but one of the wolves notice you and now you are surrounded.`
      );
      ...
      if (player2.fight1 === "Engage") {
        ...
        console.log(
          chalk.red(
            "What the hell were you aiming for? A peace treaty with a bunch of dogs? You got what you asked for, you dolt."
          )
        );
  ...
  console.log(
    "While moving through the forest, you stumble upon a cave.\nYou look at the sun and it was almost dusk."
  );
  ...
  if (player3.choice2 === "Camp") {
    ...
    console.log(
      "You spend some time chatting and trading past experiences when you suddenly hear a sharp screeching sound."
    );
    console.log(
      "You ready yourself for danger and sure enough, as you move out of the cave, you find yourself surrounded by a group of vicious goblins."
    );
  } else if (player3.choice2 === "Move ahead") {
    console.log(chalk.red("Alright then! Onward we go!"));
    status();
    console.log(
      "While you keep moving through the forest, darkness sets in and you find yourselves a little lost in the thick woods."
    );
    console.log(
      "You sense danger in every shadow and ready yourself for a fight. Sure enough, you find yourself surrounded by a group of vile goblins."
    );
  }
  ...
  for (
    goblins && player_status.health;
    goblins >= 0 && player_status.health >= 0;
    goblins && player_status.health
  ) {
    let num: number = Math.floor(Math.random() * 10);
  ...
  if (player_status.health > 0) {
    ...
    console.log(
      chalk.red("Come on lad, we must keep moving if we are to survive.")
    );
    console.log(
      "Moving through the forest, you end up in a small tribal village.\nSeeing your state, the people usher you to their herbalist.\nHere, you spend a few days recovering your health."
    );
    ...
    if (player4.leave) {
      ...
      console.log(
        "You pack your belongings and rations.\nThe herbalist provides you with fresh bandages and you thank all the villagers for their hospitality."
      );
      player_status.bandages++;
      status();
      console.log(
        "When you reached a few miles ahead, you turn around to see the a huge cloud of smoke rising from the direction of the village."
      );
      let player5 = await inquirer.prompt({
        name: "village",
        type: "confirm",
        message: chalk.red(
          "Oh no! Looks like the vilage is in trouble. Let us go and help..."
        ),
      });
      if (player5.village) {
        console.log(
          "You run back to find the village in turmoil.\nYou rush to the village borders to see trolls invading the village. The men were trying to hold them back by spears but failing miserably."
        );
      } else {
        console.log(
          chalk.red(
            "WHAT? AFTER THEY TOOK YOU IN WHEN YOU WERE BLEEDING ALONE IN THE FOREST? HOW CAN YOU SAY THIS? ARE YOU EVEN HUMAN?\nPREPARE YOURSELF FOR MY WRATH...."
          )
        );
        wizardspell();
        process.exit(0);
      ...
      console.log(
        "As you spend the time helping the villagers in their daily chores, the village warning bell starts to ring.\nYou rush to the village borders to see trolls invading the village. The men were trying to hold them back by spears but failing miserably."
      );
    }
    ...
    if (player6.trolls === "Escape") {
      ...
      console.log(
        chalk.red(
          "have you already forgotten the hospitality of the villagers? you truly have no shame...\nforget about the trolls. your death will come by my hands...".toUpperCase()
        )
      );
      wizardspell();
    } else if (player6.trolls === "Defend") {
      for (
        trolls && player_status.health;
        trolls >= 0 && player_status.health >= 0;
        trolls && player_status.health
      ) {
        let num: number = Math.floor(Math.random() * 10);
      ...
      if (player_status.health > 0) {
        console.log(
          chalk.red(
            "Ah! We survived. You killed most of the strolls and the rest of them scrambled.\nHey, are you alri...?"
          )
        );
        console.log(
          "You woke up and the wizard told you that you had passed out soon after the trolls escaped."
        );
        console.log(
          chalk.red("Had me worried. You've been passed out for three days.")
        );
        console.log(
          "You recover a day later and the bid the villagers farewell."
        );
        console.log(
          "After another day of travelling, you reach your destination\nBack to your village, home and your wife."
        );
  ...
  console.log(
    chalk.red("Ahh well! I'll just wait for another traveller then.\nFarewell!")
  );
}
