...
let id: number = Math.floor(Math.random() * 100000 + 5000);
...
if (welcome.confirmation == true) {
  ...
  if (pay.amount == courses[student.course]) {
    ...
    console.log(
      `An amount of ${pay.amount} was recieved. ${
        pay.amount - courses[student.course]
      } Rs were sent back. Application accepted.`
    );
    let studentid = {
      name: student.Name,
      fname: student.fname,
    ...
    console.log(
      "The amount is insufficient. " + pay.amount + " Rs were sent back."
    );
  }
} else {
